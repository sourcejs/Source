...
define([
    ...
    "modules/browser",
    "modules/sections",
    "modules/innerNavigation"], function($, options, utils, browser, sections, innerNavigation) {
    ...
    $(function(){
        ...
        var
                ...
                SECTION_CLASS = options.sectionClass,
        ...
        */
        ...
        var openSpoiler = function (t, config) {
            ...
            if (options.pluginsEnabled.mustache) {
                // Need to use absolute path to get same scope with requires from inline scripts
                require(['/plugins/mustache/js/mustache-lib-wrap.js'], function(templater){
        ...
        var openOnNavigation = function(hash) {
            var
                navHash = utils.parseNavHash(hash);
        ...
        if (window.history && history.pushState && !$.browser.msie)  {
            window.addEventListener('popstate', function (event) {
                openOnNavigation(window.location.hash);
            });
        } else {
            $('.source_main_nav_a').on({
                'click': function(e) {
                    ...
                    var
                        navHash = utils.parseNavHash(window.location.hash);
        ...
        innerNavigation.addMenuItem('Спрятать все секции', function(){
            for (var i = 0; i < sectionsOnPage.length; i++) {
                t = $(sectionsOnPage[i]);
                closeSpoiler(t, true);
            }
